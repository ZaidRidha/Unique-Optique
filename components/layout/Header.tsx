              ...
              {NAV_LINKS.slice(0, 4).map((link) => (
                <Link key={link.href} href={link.href} className="mx-2">
                  <motion.button
                    whileHover={{ scale: 1.05 }}
                    whileTap={{ scale: 0.95 }}
                    className="px-3 py-3 font-grotesk text-base font-medium text-[var(--color-offwhite)] hover:text-[var(--color-gold)] transition-colors relative group whitespace-nowrap"
                  >
                    {link.name}
